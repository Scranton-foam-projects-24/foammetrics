...
def index_3_gon(vertices, idx, N, M):
    ...
    # If the polygon is a triangle on the top of the lattice
...
if __name__ == "__main__":
    ...
    for m in range(0,M):
        ...
        # Construct a column of vertices which are all connected according to the
        # 3^2, 4^2 Archimedian Lattice
        for n in range(1, N):
            # In this context, rows and columns are in reference to rows and
            # columns of tiles, not vertices unless otherwise specified
            
            # Plot vertex above previous vertex and connect it to previous vertex
            idx = (N * m) + n
            ...
            if (n % 4 == 3 or n % 4 == 0) and 1 < n < N:
                ...
                if idx > N and n < N-1:
                    # If the vertex is part of both a square and a downward facing
                    # triangle on an even row
                    if n % 4 == 3:
                        polys.append(sorted([idx, idx-1, idx-N]))
                        polys.append(sorted([idx, idx-N, idx-N+1, idx+1]))
                    # Otherwise, handle the vertex of the top right corner of each
                    # square on the even row
                    else:
                        polys.append(sorted([idx, idx+1, idx-N]))
                    
                    # Connect current vertex to vertex on same row in previous col
